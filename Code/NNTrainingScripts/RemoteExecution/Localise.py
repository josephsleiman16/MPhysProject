...
def load():
    print(datatype)
    y_data = np.load("/storage/datastore-group/cmcs-dmichiel/Joseph_ML_Knots/PoL/100Beads/kymodata.npy")
...
def get_data_splits(df, testsize):
    ...
    for i in range(1,Nbeads+1):
        y_columns.append(-i)
    y_columns = y_columns.reverse()

    x = df.iloc[:, :size_input]
    y = df.iloc[:, -Nbeads:]
    ...
    if NNTYPE == "RNN":
        ...
        size3 = y_train.shape[0]
        size4 = y_test.shape[0]
        if rep == "XYZ":
        ...
        y_train = np.reshape(y_train, (size3, Nbeads, 1))
        y_test = np.reshape(y_test, (size4, Nbeads, 1))
...
def setup_RNN(RNN_hidden_top, hidden_activation, opt):
    ...
    model.add(tf.keras.layers.LSTM(RNN_hidden_top[-1], activation=hidden_activation, return_sequences=True))

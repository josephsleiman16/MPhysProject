...
def load():
    ...
    a = np.where(data==0)
    print(a)
    print("Number of empty/zero value positions in data array...")
    print(len(a[0]))
    del a
    return data
...
def get_data_splits(df, testsize):
    ...
    if NNTYPE == "RNN":
        ...
        if rep == "XYZ":
            x_train = np.reshape(x_train, (size1, Nbeads, dimensions))
            x_test = np.reshape(x_test, (size2, Nbeads, dimensions))
        elif rep == "LOCAL":
...
def setup_RNN(RNN_hidden_top, hidden_activation, opt):
...
def setup_NN(NN_hidden_top, hidden_activation, opt):
    model = tf.keras.models.Sequential()
...
def train(data):
    ...
    if NNTYPE == "FFNN":
        ...
        NN = setup_NN(NN_top, "relu", opt)
    elif NNTYPE == "RNN":
        opt = tf.keras.optimizers.Adam(learning_rate=0.00001)
        RNN_top = [100,100,100,100]
        NN = setup_RNN(RNN_top, "tanh", opt)
    print("Training...")
    history_data = NN.fit(x_train, y_train, epochs = n_epoch, batch_size = bs, verbose = 1, validation_split=1/5, callbacks=cb_list) 
    ...
    plt.savefig("MPhysPlots/acc_trainprocess" + "_" + str(problem) + "_" + str(datatype) + "_" + str(adjacent) + "_" + str(normalised) + "_" + str(NNTYPE) + ".pdf")
    plt.show()
    ...
    plt.xlabel('Epoch')
    plt.legend(['Train', 'Validation'], loc='upper left')
    plt.savefig("MPhysPlots/loss_trainprocess" + "_" + str(problem) + "_" + str(datatype) + "_" + str(adjacent) + "_" + str(normalised) + "_" + str(NNTYPE) + ".pdf")
    plt.show()
...
#     plt.xlabel('Predicted label\nAccuracy={:0.4f}; Misclass={:0.4f}'.format(accuracy, misclass))
    ...
    plt.savefig("MPhysPlots/CM-" + str(title) + ".pdf", bbox_inches='tight')
    plt.show()
    return
...
def tune(data):
    ...
    x_train,x_test,y_train,y_test = get_data_splits(df,0.1)
    print("Tuning...")
    tuner1.search(x_train, y_train, epochs=10, batch_size = bs, verbose = 1, validation_split=1/5, callbacks = cb_list)
    return tuner1
...
checkpoint_filepath = 'NN_model_best' + "_" + str(datatype) + "_" + str(problem) + "_" + str(adjacent) + "_" + str(normalised) + "_" + str(NNTYPE)
...
if datatype == "XYZ" and adjacent == True:
    ...
    print(data[0])
    if normalised == True:
        normconstant = (np.max(adjacentData) - np.abs(np.min(adjacentData)))/2
        normfactor = np.max(adjacentData) - normconstant
        normalisedAdjacent = (adjacentData - normconstant)/normfactor
        data = normalisedAdjacent
        print(data[0])
        del normconstant
        del normfactor
        del normalisedAdjacent
        del adjacentData
    data = np.reshape(data, (Nknots*Ntrials*Nknotspertrial, Nbeads*dimensions))
...
plot_confusion_matrix(cm, Knotind, title)
